...
from brix.domain import BrowserSession, BrowserTask, ControlState, CreateTask, FailureReason, TaskEvent, TaskResult, TaskStatus, utc_now
...
class TaskManager:
    def __init__(self, store: TaskStore, profiles: ProfileManager, data_root: Path, *, max_workers: int = 2, browser_factory: Any = BrowserWorker) -> None:
    ...
    async def create(self, payload: CreateTask) -> BrowserTask:
        ...
        (run_dir / "task.json").write_text(json.dumps(task.model_dump(mode="json"), indent=2), encoding="utf-8")
        await self.emit(TaskEvent(task_id=task.id, type="task.queued", message="Browser task queued"))
        await self.queue.put(task.id)
        return task
    ...
    async def _launch(self, task_id: str) -> None:
        ...
        self.store.save_task(task)
        await self.emit(TaskEvent(task_id=task.id, type="task.started", message="Starting browser worker"))
        ...
        run_dir = self.data_root / "runs" / task.id
        worker = self.browser_factory(session, self.profiles.user_data_dir(task.profile), run_dir, trace=task.trace)
        ...
        try:
            ...
            await self.emit(TaskEvent(task_id=task.id, type="browser.started", message="Chromium browser started", data={"browser_session_id": session.id}))
            if not self.backend:
                raise RuntimeError("No agent backend configured")
            await self.backend.start_task(task)
            self.store.save_task(task)
            asyncio.create_task(self._timeout(task.id, task.timeout_seconds))
        except Exception:
            await worker.close()
    ...
    async def execute_tool(self, task_id: str, action: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if action == "request_human":
            ...
            await self.request_human(task_id, reason, str(arguments.get("message", "Human interaction required")))
            return {"status": "waiting_for_user", "reason": reason}
        if action == "complete":
            await self.complete(task_id, TaskResult(success=bool(arguments["success"]), summary=str(arguments["summary"]), extracted_data=arguments.get("extracted_data") or {}, final_url=worker.session.current_url))
            return {"status": "completed"}
        try:
            risk = self.policy.enforce(task, action)
        except PermissionRequired as error:
            await self.emit(TaskEvent(task_id=task.id, type="approval.required", message=str(error), data=error.approval.model_dump(mode="json")))
            raise
        except PermissionDenied as error:
            await self.emit(TaskEvent(task_id=task.id, type="browser.permission_denied", message=str(error), data={"action": action}))
            raise
        ...
        await self.emit(TaskEvent(task_id=task.id, type=f"browser.{action}", message=f"Browser {action}", data={"risk_level": risk, "result": result}))
        if action not in {"snapshot", "detect_challenge", "assert_text", "assert_url"}:
            challenge = await worker.detect_challenge()
            if challenge["detected"]:
                await self.request_human(task_id, FailureReason(challenge["reason"]), "Human verification is required before the task can continue.")
        return result
    ...
    async def take_control(self, task_id: str) -> BrowserTask:
        ...
        self.store.save_session(worker.session); self.store.save_task(task)
        await self.emit(TaskEvent(task_id=task.id, type="browser.user_controlled", message="User took control"))
        return task

    async def return_control(self, task_id: str) -> BrowserTask:
        task, worker = self._task(task_id), self._worker(task_id)
        task.status = TaskStatus.RESUMING; self.store.save_task(task)
        ...
        self.store.save_session(worker.session); self.store.save_task(task)
        await self.emit(TaskEvent(task_id=task.id, type="task.resumed", message="Control returned to agent"))
        if self.backend:
    ...
    async def pause(self, task_id: str) -> BrowserTask:
        task, worker = self._task(task_id), self._worker(task_id)
        await worker.set_control(ControlState.PAUSED); task.status = TaskStatus.PAUSED
        self.store.save_task(task); self.store.save_session(worker.session)
        return task
    ...
    async def request_human(self, task_id: str, reason: FailureReason, message: str) -> None:
        ...
        self.store.save_task(task); self.store.save_session(worker.session)
        await self.emit(TaskEvent(task_id=task.id, type="task.waiting_for_user", message=message, data={"reason": reason, "browser_session_id": worker.session.id}))

    async def complete(self, task_id: str, result: TaskResult) -> BrowserTask:
        task = self._task(task_id); worker = self._worker(task_id)
        ...
        self.store.save_task(task)
        run_dir = self.data_root / "runs" / task.id
        (run_dir / "result.json").write_text(json.dumps(result.model_dump(mode="json"), indent=2), encoding="utf-8")
    ...
    async def cancel(self, task_id: str) -> BrowserTask:
        ...
        if self.backend: await self.backend.cancel_task(task)
        task.status, task.completed_at = TaskStatus.CANCELLED, utc_now()
        task.result = TaskResult(success=False, summary="Task cancelled", error=FailureReason.USER_CANCELLED)
        self.store.save_task(task); await self.emit(TaskEvent(task_id=task.id, type="task.cancelled", message="Task cancelled")); await self._release(task)
        return task

    async def fail(self, task_id: str, reason: FailureReason, message: str) -> None:
        task = self.store.get_task(task_id)
        if not task or task.status in TERMINAL: return
        task.status, task.completed_at = TaskStatus.FAILED, utc_now()
        task.result = TaskResult(success=False, summary="Browser task failed", error=f"{reason}: {message}")
        self.store.save_task(task); await self.emit(TaskEvent(task_id=task.id, type="task.failed", message=message, data={"reason": reason})); await self._release(task)

    async def _timeout(self, task_id: str, seconds: int) -> None:
        await asyncio.sleep(seconds)
        task = self.store.get_task(task_id)
        if task and task.status not in TERMINAL:
            task.status = TaskStatus.TIMED_OUT; self.store.save_task(task)
            await self.emit(TaskEvent(task_id=task.id, type="task.timed_out", message="Task timed out")); await self._release(task)
    ...
    async def emit(self, event: TaskEvent) -> None:
        ...
        await asyncio.gather(*(callback(event) for callback in self.subscribers[event.task_id]), return_exceptions=True)

    def subscribe(self, task_id: str, callback: Subscriber) -> None: self.subscribers[task_id].add(callback)
    def unsubscribe(self, task_id: str, callback: Subscriber) -> None: self.subscribers[task_id].discard(callback)
    def _task(self, task_id: str) -> BrowserTask:
        task = self.store.get_task(task_id)
        if not task: raise KeyError(task_id)
        return task
    def _worker(self, task_id: str) -> BrowserWorker:
        if task_id not in self.workers: raise KeyError(f"No active browser for {task_id}")
        return self.workers[task_id]

    async def handle_codex_event(self, payload: dict[str, Any]) -> None:
        params = payload.get("params", {})
        thread_id = params.get("threadId")
        task = next((item for item in self.store.list_tasks() if item.codex_thread_id == thread_id), None)
        if task:
            await self.emit(TaskEvent(task_id=task.id, type="agent.event", message=str(payload.get("method", "Codex event")), data={"event": payload}))
