...
class Store:
    ...
    @staticmethod
    def _json(model: BrixSession | Agent | AgentReport) -> str:
        return model.model_dump_json()

    def save_session(self, session: BrixSession) -> BrixSession:
    ...
    def get_session(self, session_id: str) -> BrixSession | None:
        with self.connection() as db:
            row = db.execute("SELECT data FROM sessions WHERE id = ?", (session_id,)).fetchone()
        return BrixSession.model_validate_json(row["data"]) if row else None

    def list_sessions(self) -> list[BrixSession]:
        with self.connection() as db:
            rows = db.execute("SELECT data FROM sessions ORDER BY created_at DESC").fetchall()
        return [BrixSession.model_validate_json(row["data"]) for row in rows]
