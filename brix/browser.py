...
class BrowserWorker:
    ...
    def __init__(self, session: BrowserSession, user_data_dir: Path, run_dir: Path, *, trace: bool = False) -> None:
    ...
    async def start(self) -> None:
        try:
            ...
            raise BrowserError("Playwright is not installed; run `playwright install chromium`") from error
        ...
        self._context = await self._playwright.chromium.launch_persistent_context(
            str(self.user_data_dir), headless=False, accept_downloads=True,
            downloads_path=str(self.run_dir / "downloads"), viewport={"width": 1440, "height": 900},
        )
        if self.trace:
    ...
    async def execute(self, action: str, arguments: dict[str, Any]) -> dict[str, Any]:
        async with self._action_lock:
            ...
            if action == "navigate":
                response = await page.goto(str(arguments["url"]), wait_until="domcontentloaded", timeout=30_000)
            ...
            if action == "scroll":
                ...
                return {"ok": True}
            if action == "tabs":
                return {"tabs": [{"index": i, "url": tab.url, "title": await tab.title()} for i, tab in enumerate(self._context.pages)]}
            ...
            if action == "screenshot":
                ...
                await page.screenshot(path=str(path), full_page=bool(arguments.get("full_page", False)))
    ...
    async def snapshot(self) -> dict[str, Any]:
        page = await self._guard()
        raw = await page.locator("a,button,input,textarea,select,[role],[contenteditable=true]").evaluate_all(
            """els => els.slice(0, 250).map((el, i) => ({index:i, tag:el.tagName.toLowerCase(),
            role:el.getAttribute('role') || ({A:'link',BUTTON:'button',INPUT:'textbox',TEXTAREA:'textbox',SELECT:'combobox'})[el.tagName] || el.tagName.toLowerCase(),
            name:el.getAttribute('aria-label') || el.labels?.[0]?.innerText || el.innerText || el.getAttribute('placeholder') || el.getAttribute('name') || '',
        ...
        for item in raw:
            ...
            elements.append({"id": element_id, "role": item["role"], "name": str(item["name"]).strip()[:300], "visible": item["visible"]})
    ...
    async def detect_challenge(self) -> dict[str, Any]:
        ...
        selectors = await page.locator("iframe[src*='captcha'], [class*='captcha'], [id*='captcha']").count()
        if selectors or any(term in content for term in ("captcha", "verify you are human", "cloudflare challenge", "turnstile")):
            return {"detected": True, "reason": FailureReason.CAPTCHA}
        if any(term in content for term in ("one-time code", "verification code", "authenticator", "passkey", "security code")):
